...
Add a new canvas object called `GUI` to your scene.
...
1. Now we need to make a template for the speed labels, to begin with, create an empty game object child named `SpeedLabelTemplate` of the `Speedometer` game object we previously added, this will act as a pivot point for our speed labels, add all the following as child objects.

- Create an UI image game object setting its width to **2** and height to **10**, and setting the _Pos Y_ position to **140** to place it above the pivot point. This will act as a dashed line for each of our speed labels.

- Add a text UI element named `speedText` setting its text property to **100** for the template, its font size to **16**, and its text alignment to middle and center. Under its Rect Transform set its Pos Y to **120** to position it next to the dash line.\
...
2. Next we need to add the needle to our speedometer, start off by adding a UI image game object named `NeedleIMG` as a child of and centered to the `Speedometer` game object, setting its width and height to **125**.
...
- To be able to have our needle rotate around the speedometer's center properly we will need to change its pivot point, to do this simply go to the Rect Transform inspector properties of the `NeedleIMG` and set its Pivot Y property to **0**, this will then change the Pos properties which we need to set back to 0,0,0.\
...
- To begin with, we need a reference to the object we want to measure the speed of, we can do this by declaring a _rigidbody_ variable named `target`, which will be used to read the velocity of our target object (_only if your target game object is using a rigidbody_). **Make sure to add `[SerializeField]` before the variable so it can be set in the inspector.**
```
  [SerializeField] Rigidbody target;
```
- We need a property/variable to control the maximum speed we want our speedometer to display up to, this is required to calculate the maximum angle the needle can turn and the last speed label to generate and display...
```
...
- Also we need another two variables to reference and control the needle and speed label template game objects
```
...
- Now in the update method we can use the function we previously created to rotate the needle relative to the speed...
```
void Update()
{
...
3. Next let's develop some code to generate our speed labels

- Starting we need to add an extra variable, an int called `speedLabelAmount` to control how many speed labels to generate
```
...
- Inside our new method add a for loop making sure its condition (2nd statement) is `i <= speedLabelAmount` and not just the default default operator `<` as this will prevent the last label being generated...
```
void CreateSpeedLabels(float maxSpeed){
...
```
    ...
    float labelSpeedNormalized = (float)i / speedLabelAmount; //Calculates a percentage index relative to the label amount for the generated speed label to use to calculate its position and display speed
...
## 4. Testing the analogue speedometer
...
- We need to add some new variables to reference the new game objects we want to manipulate
```
...
- Now let's create a new float method to calculate our desired speed unit measure taking in the target's velocity as an argument...
```
...
Your update method should look like the below
```
void Update()
    {
...
- Go back to unity and select the `Speedometer` object
  - In its inspector properties select your desired speed unit in the `Speed Type` dropdown
